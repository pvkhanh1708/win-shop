                        ...
                        @if(sizeof($product->images) > 0)
                            <div class="product__details__pic__slider owl-carousel">
                                @foreach($product->images as $item)
                                    <img data-imgbigurl="{{url($item->url)}}"
                                         src="{{url($item->url)}}" alt="">
                                @endforeach
                            </div>
                        @endif
                    </div>
                ...
                <div class="col-lg-7 col-md-7">
                    <div class="product__details__text">
                        ...
                        <div class="product__details__rating">
                            ...
                            <span>({{ $product->reviews?$product->reviews->count():0 }} {{ trans('page.reviews') }})</span>
                        </div>
                        @if($product->sale_price > 0)
                            <div class="product__details__price"><stroke>{{number_format($product->cost_price)}} {{ trans('page.unit_currency') }}</stroke></div>
                            <div class="product__details__price">{{number_format($product->sale_price)}} {{ trans('page.unit_currency') }}</div>
                        @else
                            <div class="product__details__price">{{number_format($product->cost_price)}} {{ trans('page.unit_currency') }}</div>
                        @endif
                        ...
                        <div class="row-action">
                            ...
                            <a id="add_to_wish" data-id="{{ $product->id }}" class="heart-icon @if(Auth::user() && \App\Models\Wish::where('user_id', Auth::user()->id)->where('product_id', $product->id)->first() != null) active @endif">
                                ...
                                <span>
                                    @if(isset($product->categories) && $product->categories->count() > 0)
                                        @foreach($product->categories as $cat)
                                            <a href="{{ url('category/'.$cat->category->slug) }}" class="mr-1 badge bg-success">{{ $cat->category->name }}</a>
                                        @endforeach
                                    @else
                                        <span class="bg-warning">{{ trans('page.no_category') }}</span>
                                    @endif
                                </span>
                            </li>
                            <li>

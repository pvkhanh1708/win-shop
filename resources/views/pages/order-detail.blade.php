                                ...
                                @foreach($order->products as $item)
                                    <tr class="row-product">
                                        ...
                                        <td class="shoping__cart__price">
                                            {{ $item->product_price > 0 ? number_format($item->product_price) : 0 }} {{ trans('page.currency') }}
                                        </td>
                                        <td class="shoping__cart__quantity">
                                            <b>{{ $item->quantity }}</b>
                                        </td>
                                        <td class="shoping__cart__total">
                                            {{ $item->total ? number_format($item->total * $item->quantity) : 0 }} {{ trans('page.currency') }}
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
                                ...
                                <tr class="row-product my-3">
                                    ...
                                    <td><h4><b class="text-danger">TỔNG</b></h4></td>
                                    <td><h4><b class="text-danger">{{ number_format($order->products()->sum('quantity')) }}</b></h4></td>
                                    <td><h4><b class="text-danger">{{ number_format($order->sub_total) }}  {{ trans('page.currency') }}</b></h4></td>
                                </tr>
                            </tbody>
                ...
                <div class="col-lg-12">
                    <div class="row">
                        <div class="col-md-6 col-xs-12">
                            ...
                            <ul>
                                <li>Tên người nhận: <b class="float-right">{{ $order->address->customer_name }}</b></li>
                                <li>Số điện thoại: <b class="float-right">{{ $order->address->phone }}</b></li>
                                @if($order->address->email)
                                    <li>Email: <b class="float-right">{{ $order->address->email }}</b></li>
                                @endif
                                <li>Địa chỉ: <b class="float-right">{{ $order->address->address }}, {{ $order->address->ward }}, {{ $order->address->district }}, {{ $order->address->province }}</b></li>
                            </ul>
                            @else
                        ...
                        <div class="col-md-6 col-xs-12">
                            ...
                            <ul>
                                ...
                                <li>Trạng Thái: <b class="float-right">@if($order->payment->status==1) Đã Thanh Toán @else Chưa Thanh Toán @endif</b></li>
                            </ul>
                            @endif
                        </div>
                    </div>

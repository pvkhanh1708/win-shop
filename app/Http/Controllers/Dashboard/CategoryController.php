...
class CategoryController extends Controller
{
    ...
    public function post_view(Request $request)
    {
        if($request->id){
            $category = Categories::find($request->id);
            if($category){
                return view('dashboards.ajax.view-category', ['category'=>$category]);
            }
            return 0;
        }
        return 0;
    ...
    public function post_edit(Request $request)
    {
        if($request->name){
            ...
            if($category){
                ...
                $category->save();
                return response()->json(array('status'=>1, 'msg'=> 'Sửa danh mục thành công!'), 200);
            }
        ...
        if($request->id){
            $category = Categories::find($request->id);
            if($category){
                return view('dashboards.ajax.edit-category', ['category'=>$category]);
            }
            return 0;
        }
        return 0;
